...
draft = (await openapiGenerate(draft, {
  generateSet: {
    operationIdTemplate: '<method>_<pathPart1>_<pathPart2>'
      ...
      title='filter-generate.js'
    />

    <h2 className='text-2xl font-semibold mt-8 mb-4'>File Helpers</h2>
    <p className='mb-4 text-muted-foreground'>
      The module exports smart parsing and writing helpers with support for large numbers, YAML
      comments, and remote loading:
    </p>
    <CodeBlock
      code={`const {
...
await writeFile('openapi.sorted.yaml', output, { format: 'yaml' });`}
    ...
    <div className='mt-6 rounded-lg border bg-muted/30 p-4 text-sm text-muted-foreground'>
      ...
      <p>
        ...
        <code className='rounded bg-muted px-1 py-0.5 font-mono text-xs'>sortComponentsSet</code>{' '}
