...
const SortingPage = () => (
  <DocsLayout>
    ...
    <p className="mb-4 text-muted-foreground">Create a JSON or YAML file specifying your preferred field order per key:</p>
    <CodeBlock code={`{
  ...
  "schema": ["type", "description", "properties", "required"]
    ...
    <p className="mb-4 text-muted-foreground">
      Control the order of paths in your OpenAPI document using <code className="rounded bg-muted px-1.5 py-0.5 font-mono text-sm">sortPathsBy</code>:
    </p>
    ...
    <div className="overflow-x-auto mb-6">
      <table className="w-full text-sm border-collapse">
        ...
        <tbody className="text-muted-foreground">
          <tr className="border-b"><td className="py-2 pr-4 font-mono text-xs text-foreground">original</td><td className="py-2">Keep the original order (default)</td></tr>
          <tr className="border-b"><td className="py-2 pr-4 font-mono text-xs text-foreground">path</td><td className="py-2">Order alphabetically by path</td></tr>
          <tr className="border-b"><td className="py-2 pr-4 font-mono text-xs text-foreground">tags</td><td className="py-2">Order by the first tag of the first method</td></tr>
        </tbody>
      </table>
    </div>

    <h2 className="text-2xl font-semibold mt-8 mb-4">Sort Components</h2>
    <p className="mb-4 text-muted-foreground">Sort items within the components section alphabetically:</p>
    <CodeBlock code={`npx openapi-format openapi.json -o output.json --sortComponentsFile sortComponents.json`} title="Terminal" />
    <CodeBlock code={`["schemas", "parameters", "headers", "requestBodies", "responses", "securitySchemes"]`} title="sortComponents.json" />
  ...
  schemas:
    ...
    Customer:
      type: object
    Order:
      type: object`} title="After" />
    </div>

    <h2 className="text-2xl font-semibold mt-8 mb-4">Sort Component Properties</h2>
    <p className="mb-4 text-muted-foreground">Sort properties within schema components alphabetically:</p>
    <CodeBlock code={`npx openapi-format openapi.json -o output.json --sortComponentsProps`} title="Terminal" />

    <div className="grid gap-4 md:grid-cols-2">
      <CodeBlock code={`schemas:
  UserDto:
    type: object
    properties:
      lastName:
        type: string
      firstName:
        type: string`} title="Before" playground />
      <CodeBlock code={`schemas:
  UserDto:
    type: object
    properties:
