...
const GeneratePage = () => (
  <DocsLayout>
    ...
    <p className='text-muted-foreground mb-8'>
      Automatically generate OpenAPI elements like{' '}
      <code className='rounded bg-muted px-1.5 py-0.5 font-mono text-sm'>operationId</code> using
      customizable templates.
    </p>

    <h2 className='text-2xl font-semibold mt-8 mb-4'>CLI Usage</h2>
    <CodeBlock
      code={`npx openapi-format openapi.json -o output.json --generateFile customGenerate.yaml`}
      title='Terminal'
    />

    <h2 className='text-2xl font-semibold mt-8 mb-4'>Configuration</h2>
    <CodeBlock
      code={`operationIdTemplate: "<method>_<pathPart2>"
overwriteExisting: false`}
      title='customGenerate.yaml'
    />
    ...
    <div className='overflow-x-auto mb-8'>
      <table className='w-full text-sm border-collapse'>
        ...
        <tbody className='text-muted-foreground'>
          <tr className='border-b'>
            <td className='py-2 pr-4 font-mono text-xs text-foreground'>operationIdTemplate</td>
            <td className='py-2'>Template with dynamic placeholders for generating operationId</td>
          </tr>
          <tr className='border-b'>
            <td className='py-2 pr-4 font-mono text-xs text-foreground'>overwriteExisting</td>
            <td className='py-2'>Whether to overwrite existing values (default: false)</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <CodeBlock
      code={`operationIdTemplate: "<method>_<pathPart1>_<pathPart2>"
overwriteExisting: false`}
      title='customGenerate.yaml'
    />
  ...
  /crm/leads/{leadId}:
    ...
    put:
      ...
      summary: Update a lead`}
        title='Before — missing operationIds'
        playground
      />
      <CodeBlock
        code={`openapi: 3.0.3
paths:
  ...
  /crm/leads/{leadId}:
    ...
    put:
      operationId: updateLead
      summary: Update a lead`}
        title='After — generated (existing kept)'
      />
    </div>

    <h2 className='text-2xl font-semibold mt-8 mb-4'>Example: Overwrite All operationIds</h2>
    <CodeBlock
      code={`operationIdTemplate: "<method>-<pathPart1>-<pathPart2>"
