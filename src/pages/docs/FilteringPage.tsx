...
const FilteringPage = () => (
  <DocsLayout>
    ...
    <CodeBlock
      code={`npx openapi-format openapi.json -o output.json --filterFile customFilter.yaml`}
      title='Terminal'
    />
    <CodeBlock
      code={`flags:
...
operationIds:
  ...
  - findPetsByStatus`}
    ...
    <p className='mb-4 text-muted-foreground'>
      Remove all operations matching specified HTTP methods. Use{' '}
      <code className='rounded bg-muted px-1.5 py-0.5 font-mono text-sm'>inverseMethods</code> to
...
paths:
  /pets:
    post:
      summary: Add a new pet`}
        title='After — GET and PUT removed'
      />
    </div>
  ...
  /users:
    get:
      tags:
        - user
      summary: List all users`}
        title="After — 'pet' operations removed"
      />
    </div>

    <h2 className='text-2xl font-semibold mt-8 mb-4'>Filter by operationIds</h2>
    <p className='mb-4 text-muted-foreground'>
      Remove specific operations by their{' '}
      <code className='rounded bg-muted px-1.5 py-0.5 font-mono text-sm'>operationId</code>:
    </p>
    <CodeBlock
      code={`operationIds:
  - findPetsByStatus`}
...
paths:
  /pets:
    post:
      operationId: addPet
      summary: Add a new pet`}
        title='After — findPetsByStatus removed'
      />
    </div>

    <h2 className='text-2xl font-semibold mt-8 mb-4'>Filter by Operations</h2>
    <p className='mb-4 text-muted-foreground'>
      Target specific method + path combinations using the{' '}
      <code className='rounded bg-muted px-1.5 py-0.5 font-mono text-sm'>METHOD::path</code> format.
      Supports wildcards:
    </p>
    <CodeBlock
      code={`operations:
...
paths:
  ...
  /users:
    get:
      summary: List all users`}
        title="Before (operations: ['PUT::/pets'])"
        playground
      />
      <CodeBlock
        code={`openapi: 3.0.0
...
paths:
  ...
  /users:
    get:
      summary: List all users`}
        title='After — PUT /pets removed'
      />
    </div>

    <h2 className='text-2xl font-semibold mt-8 mb-4'>Filter by Flags</h2>
    <p className='mb-4 text-muted-foreground'>
      Remove items that have a specific custom property (e.g.{' '}
      <code className='rounded bg-muted px-1.5 py-0.5 font-mono text-sm'>x-internal</code>):
    </p>
    <CodeBlock
      code={`flags:
  - x-internal`}
...
paths:
  /pets:
    get:
      summary: List all pets`}
        title='After — internal endpoints removed'
      />
    </div>
...
paths:
  /pets:
    post:
      x-version: 2.0
      summary: Add pet (v2)`}
        title='After — v1.0 and v3.0 removed'
      />
    </div>
        ...
        '200':
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/Pet'`}
        title='After — XML content removed'
      />
    </div>

    <h2 className='text-2xl font-semibold mt-8 mb-4'>Remove Unused Components</h2>
    <p className='mb-4 text-muted-foreground'>
      Strip unreferenced items from the components section. Recursively removes nested unused
      components (up to 10 levels).
    </p>
    <CodeBlock
      code={`unusedComponents:
  - schemas
  - parameters
  - examples`}
      title='filter.yaml'
...
components:
  schemas:
    Pet:
      type: object`}
        title='After — Error & Category removed (unused)'
      />
    </div>
  ...
  /pets:
    get:
      summary: Finds Pets
      operationId: findPets`}
        title='After — flags removed, operation kept'
      />
    </div>
