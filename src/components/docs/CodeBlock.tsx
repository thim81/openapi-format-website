...
function inferLanguage(title?: string, code?: string): string {
  ...
  if (code) {
    ...
    if (code.includes("openapi-format ") || code.includes("npx ") || code.includes("npm ") || code.startsWith("$")) return "bash";
  }
  return "yaml";
}
...
const CodeBlock = ({ code, language, title, playground }: CodeBlockProps) => {
  ...
  const langMap: Record<string, string> = {
    sh: "bash", shell: "bash", terminal: "bash",
      ...
      <div className="rounded-lg border bg-muted/30 overflow-hidden">
        {title && (
          <div className="flex items-center justify-between border-b px-4 py-2 text-xs text-muted-foreground">
            <span>{title}</span>
            <button onClick={copy} className="hover:text-foreground transition-colors" aria-label="Copy">
